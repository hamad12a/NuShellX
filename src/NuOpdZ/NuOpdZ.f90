...
character(len=12):: z1
integer::  trdd,trmd,trddu,max_lm,Tparity1,Tparity2,totlms,totlm,nelm,nel,llsp
logical:: proton,neutron
real:: x1,x2
...
11  print *, ' EOF reading prjr file '
    ...
    integer:: min_shells,max_shells,maxlmm=0,lmt,sh1,sh2,sh3,sh4,par
! open TRM file and deduce types
 
    allocate(csj2(0:no_shells))
    allocate(lsp(0:max_sj2u,no_shells,no_shells,2))
    lsp=.false.
    csj2=0
    do lsc=1,no_shells
        csj2(lsc)=sj2(lsc)
    ...
    if ((ftypea.and.typea=='p').or.(ftypeb.and.typeb=='p')) then
        lsp(lm/2,sh1,sh2,(par+3)/2)=.true.
    else if ((ftypeb.and.typeb=='n').or.(ftypea.and.typea=='n')) then
        lsp(lm/2,sh3,sh4,(par+3)/2)=.true.
    end if
    end do
    end do
    close(unit=43)
...
170 if (trmd/=lsc) then
    ...
    do nsc=1,trmd
        ...
        par=2
        if (btest(sl(shl)+sl(shr),0)) par=1
        do lm=lmn,lmx,2
            if ((lsp(lm/2,shl,shr,par).and.ltrd).or..not.ltrd) then
            ta(1:5)=tm(1:5)
            ta(ilm:ilm)=char(lm)
            ta(6:12)=z1(6:12)
...
54  end do
    ...
    if (allocated(trma)) deallocate(trma)
    allocate(trdao(trdd))
