...
character(len=12):: z1
integer::  trdd,trmd,trddu,max_lm,Tparity1,Tparity2
...
11  print *, ' EOF reading prjr file '
    ...
    integer:: min_shells,max_shells,maxlmm=0,lmt
...
170 if (trmd/=lsc) then
    ...
    do nsc=1,trmd
        ...
        lmn=max(abs(lmm),abs(csj2(shr)-csj2(shl)),abs(jl-jr))
        lmx=min(abs(csj2(shr)+csj2(shl)),abs(jl+jr),max_lm)
        do lm=lmn,lmx,2
            ta(1:5)=tm(1:5)
            ta(ilm:ilm)=char(lm)
            ta(6:12)=z1(6:12)
