...
real:: xa,xb,xxa,xxb
integer:: totlm,max_elm,max_lel
integer,dimension(:,:,:),allocatable:: lmel
integer,dimension(:),allocatable:: mapn,manp
end module Globals
...
contains
    ...
    subroutine coefor(nplra,trdao,da02,nplrao,trdan,nplran,nsa,dimo,dimx,nJT,ajl,ajr)
    implicit none
    real(kind=rkw),dimension(:),intent(inout)::trdao
    real(kind=rkw),dimension(:),intent(in)::trdan
    type(la),intent(in):: nplrao,nplran,nplra
    integer,intent(in):: dimx,nsa,ajl,ajr,da02
    integer,intent(inout)::dimo
    integer,dimension(:,0:),intent(in):: nJT
    real(kind=rkw),dimension(da02):: trdat
    integer:: dim,po,pn,pon
    integer:: i,j,iw,ib,word
    logical:: done
    trdat=real(0,rkw)
    po=0
    pn=0
...
5   call next_ij(nplra,i,j,nsa,nsa,word,iw,ib,done)    
    if  (done) then
        if (pon>da02) stop '  pon error'
        dimo=pon
        trdao=trdat
        return
    end if
    if ((i.and.nplrao%lvect(j)).and.(i.and.nplran%lvect(j)))  then
    dim=nJT(j,ajr/2)*nJT(i,ajl/2) 
    trdat(pon+1:pon+dim)=trdao(po+1:po+dim)+trdan(pn+1:pn+dim)
    pn=pn+dim
    po=po+dim
    pon=pon+dim
    if (po>dimo) stop '  po error '
    if (pn>dimx) stop '  pn error '
    if (pon>da02) stop '  pon error '
    go to 5
    else if (i.and.nplran%lvect(j)) then    
    dim=nJT(j,ajr/2)*nJT(i,ajl/2)     
    trdat(pon+1:pon+dim)=trdan(pn+1:pn+dim)
    pon=pon+dim
    if (pon>da02) stop '  pon error '
    pn=pn+dim
    if (pn>dimx) stop '  pn error '
    go to 5
    else if (i.and.nplrao%lvect(j)) then
    dim=nJT(j,ajr/2)*nJT(i,ajl/2)
    trdat(pon+1:pon+dim)=trdao(po+1:po+dim)
    pon=pon+dim
    if (pon>da02) stop '  pon error '
    po=po+dim
    if (po>dimo) stop '  po error '
    ...
    print *, ajl,ajr,i,j,pon,da02,po,dimo,pn,dimx
    stop '  coefor error'
    
    end subroutine coefor
...
! read opn         
    ...
    if (abs(opmeV(psc))<0.000001) opmeV(psc)=real(0,rkw)
...
! read trda
    ...
    do lsc=nsc+1,nsc+dima(atsc)
        if (abs(trdav(lsc))<.000001) trdav(lsc)=0.0
    end do
...
! read trdb
    ...
    do lsc=nsc+1,nsc+dimb(btsc)
        if (abs(trdbv(lsc))<.000001) trdbv(lsc)=0.0
    end do
...
!DEC$ IF DEFINED (matrix)
   ...
   implicit none
   real(kind=rkw),dimension(:,:),allocatable::matrixc
   real(kind=rkw),dimension(:),allocatable::vector
   real(kind=rkw):: E
   integer:: i,j,no_levels,no_st
   character(len=7):: operat

   call get_command_argument(5,operat)   
   
   open(unit=903,file=Nucleuss//file_code(J2/2,no_cA,no_cB)//'.xvc',form='unformatted')
   open(unit=904,file=Nucleuss//file_code(J2/2,no_cA,no_cB)//'.xvc',form='unformatted')
   open(unit=905,file=Nucleuss//file_code(J2/2,no_cA,no_cB)//trim(operat)//'.ovl',action='write')
   open(unit=906,file=Nucleuss//file_code(J2/2,no_cA,no_cB)//trim(operat)//'.ovx',&
            form='unformatted',action='write')
   call output_headerX(905)
   read(903) no_st,no_levels
   read(904) no_st,no_levels
   allocate(matrixc(no_levels,no_levels))
   matrix=real(0,rkw)
   do i=1,no_levels
   read(903,end=99) E,coefR
   call multiplyx()
   call multiplya()
   call multiplyb()
   do j=1,no_levels
   read(904,end=90) E,coefT
   matrixc(i,j)=dot_product(coefL,coefT)
90 end do    
   rewind(904)
   read(904) no_st,no_levels
99 end do
   
   write(905,*) ' Expectation Values and Off Diagonal Matrix Elements'
   write(905,*)
   write(905,*) ' Interaction/Operator file ',operat
   write(905,*)
   write(905,*) ' Initial levels from 1 ... No_Levels left to right'
   write(905,*) ' Final   levels from 1 ... No_Levels top  to bottom'
   write(905,*)
   
   write(906) no_levels
   do j=1,no_levels
   write(905,'(8f10.4)') matrixc(1:no_levels,j)
   write(906) matrixc(1:no_levels,j)
   write(905,*)
   end do
   
   write(6,*) ' Overlap matrix saved to ',Nucleuss//file_code(J2/2,no_cA,no_cB)//trim(operat)//'.ovl'
   write(6,*)
...
!DEC$ ENDIF
   ...
   dnb=sum(nJTb(1:n,jb/2))
    ...
    integer:: nja2,xja2,njb2,xjb2,dja2,djb2,par
    ...
    do par=1,2
    do ashr=1,max_elm
    lm=lmel(ashl,ashr,par)
    if (lm/=-1) then
        if (ajr>=abs(ajl-lm).and.ajr<=ajl+lm) then   
        ta(isl:isl)=char(ashr)
        ta(isr:isr)=char(200+par)
        ta(ijl:ijl)=char(ajl)
        ta(ijr:ijr)=char(ajr)
        ta(ilm:ilm)=char(lm)
        ta(6:8)=z1(6:8)
        ...
        if (bjr>=abs(bjl-lm).and.bjr<=bjl+lm) then   
            tb(isl:isl)=char(ashr)
            tb(isr:isr)=char(200+par)
            tb(ijl:ijl)=char(bjl)
            tb(ijr:ijr)=char(bjr)
            tb(ilm:ilm)=char(lm)
            tb(6:8)=z1(6:8)
            ...
            if (store==2) then
                 ...
                 tablebh(bsc,hsc)%bme(tablebhn(bsc,hsc))%me(bn)=racahs(rsc)
            end if
        end if
90      end do
        end if
...
100 end do
   ...
   subroutine prd()
    implicit none
    type(la),dimension(:),allocatable:: nplrp
    integer:: min_ashells,max_ashells,store,nn,sts,min_bshells,max_bshells,bn,tn
    integer:: equ,prdda=0,prddat=0,no_bshells,no_ashells,nelm,nle,nel,ne,totlms,par
    logical:: new,done
    real(kind=4):: e
    integer,dimension(:),allocatable:: dimp,posp
    type(vect),dimension(:),allocatable:: prdav
    character(len=7):: intfileo
    totlm=0
    max_elm=0
    max_lel=0
    call reset_string_order()
    ...
    min_bshells=min_shellsb()
    max_bshells=max_shellsb()
    no_ashells=max_ashells-min_ashells+1
    no_bshells=max_bshells-min_bshells+1
    allocate(dimp(int(scale*trdda)))
    allocate(posp(int(scale*trdda)))
    allocate(prdav(int(scale*trdda)))
    allocate(nplrp(int(scale*trdda)))
    call get_command_argument(5,intfileo)
    intfileo=adjustr(intfileo)
    open(unit=21,file=intfileo//'.dop')
    do ajl=min_cJ2a,max_cJ2a,del_cJ2a       
    do ajr=min_cJ2a,max_cJ2a,del_cJ2a       
10  read(21,*) totlms
    totlms=totlms/2+1
    ...
    read(21,*) lm,par
    read(21,*) nelm
    max_elm=max(max_elm,nelm)
    do nel=1,nelm
    read(21,*) e
    read(21,*) nle
    max_lel=max(max_lel,nle)
    do ne=1,nle
    if (typea=='p') read(21,*) ashl,ashr,bshl,bshr,me
    if (typea=='n') read(21,*) bshl,bshr,ashl,ashr,me
        if (ajr>=abs(ajl-lm).and.ajr<=ajl+lm) then   
        ...
        if (atsc==0) go to 100
            tb=ta
            tb(isl:isl)=char(nel)
            tb(isr:isr)=char(200+(par+3)/2)
            call add_string(tb,new,equ)
            if (new) then 
                prdda=prdda+1
                if ( prdda>int(scale*trdda)) then
                print *, prdda,trdda
                stop '  prda dimension error'
                end if
                call allocatela(nplrp(prdda),no_sparta,no_sparta)
                allocate(prdav(prdda)%v(da0(ajl/2)*da0(ajr/2)))
                prdav(prdda)%v=real(0,rkw)
                dimp(prdda)=dima(atsc)
                prdav(prdda)%v(1:dima(atsc))=e*renorm*me*trdav(posa(atsc)+1:posa(atsc)+dima(atsc))
                call zerola(nplrp(prdda),no_sparta)
                call laor(nplrp(prdda),nplra(atsc),no_sparta,no_sparta)
            else if (.not.new.and.equ/=0) then
                call zerola(nplrao(1),no_sparta)
                call laor(nplrao(1),nplrp(equ),no_sparta,no_sparta)
                call laor(nplrao(1),nplra(atsc),no_sparta,no_sparta)
                call coefor(nplrao(1),prdav(equ)%v,da0(ajl/2)*da0(ajr/2),&
                    nplrp(equ),e*renorm*me*trdav(posa(atsc)+1:posa(atsc)+dima(atsc)),nplra(atsc),&
                        no_sparta,dimp(equ),dima(atsc),nJTa,ajl,ajr)
                call laor(nplrp(equ),nplra(atsc),no_sparta,no_sparta)
            end if 
        end if    
100  end do
     end do 
...
500  rewind(unit=21)
    ...
    close(unit=21)
    open(unit=21,file=Nucleus//file_code(J2/2,no_cA,no_cB)//'.prd',form='unformatted',&
            status='unknown',action='readwrite')
    write(21) prdda,dimp(1:prdda)
    prddat=0
    do psc=1,prdda
        posp(psc)=prddat
        prddat=prddat+dimp(psc)
        write(21) prdav(psc)%v(1:dimp(psc))
        if (allocated(prdav(psc)%v)) deallocate(prdav(psc)%v)
        write(21) (nplrp(psc)%lvect(anpl)%lword(1:nwordsa),anpl=1,no_sparta)
        call deallocatela(nplrp(psc),no_sparta)
    end do 
    write(21) posp(1:prdda)
    rewind(unit=21)
    deallocate(prdav)
    deallocate(nplrp)
    deallocate(dimp)
    ...
    deallocate(posa)
    do psc=1,trdda
        call deallocatela(nplra(psc),no_sparta)
    end do    
    deallocate(dima)
    deallocate(nplra)
    deallocate(trdav)
    if (prdda/=0) then
    allocate(trda(prdda))
    allocate(trdav(prddat))
    allocate(mapn(prdda))
    allocate(dima(prdda))
    allocate(posa(prdda))
    allocate(nplra(prdda))
    done=.true.
    idx=0
    nsc=0
530 idx=get_next_string(done)
    nsc=nsc+1
    trda(nsc)(1:8)=string_order(idx)%string(1:8)
    mapn(nsc)=idx
    if (done) go to 540
    go to 530
540 if (prdda/=nsc) then
        print *, ' prdda,nsc ',prdda,nsc
        stop '  prdda /= nsc  '
    end if
    end if
    read(21) trdda,dima(1:trdda)
    nsc=0
    do psc=1,trdda
        posa(psc)=nsc
        read(21) trdav(nsc+1:nsc+dima(psc))
        nsc=nsc+dima(psc)
        call allocatela(nplra(psc),no_sparta,no_sparta)
        read(21) (nplra(psc)%lvect(anpl)%lword(1:nwordsa),anpl=1,no_sparta)
    end do
    deallocate(dima)
    close(unit=21)    
    prdda=0
    prddat=0
    allocate(dimp(int(scale*trddb)))
    allocate(posp(int(scale*trddb)))
    allocate(prdav(int(scale*trddb)))
    allocate(nplrp(int(scale*trddb)))
    allocate(lmel(totlms,max_elm,2))
    lmel=-1
    call reset_string_order()
    open(unit=21,file=intfileo//'.dop')
    do bjl=min_cJ2b,max_cJ2b,del_cJ2b       
...
560 read(21,*) totlms
    ...
    read(21,*) nelm
    do nel=1,nelm
    lmel(lm/2+1,nel,(par+3)/2)=lm
    read(21,*) e
    read(21,*) nle
    do ne=1,nle
    if (typea=='p') read(21,*,end=900) ashl,ashr,bshl,bshr,me
    if (typea=='n') read(21,*,end=900) bshl,bshr,ashl,ashr,me
        if (bjr>=abs(bjl-lm).and.bjr<=bjl+lm) then   
        ...
        if (btsc==0) go to 600
            ta=tb
            ta(isl:isl)=char(nel)
            ta(isr:isr)=char(200+(par+3)/2)
            call add_string(ta,new,equ)
            if (new) then 
                prdda=prdda+1
                if ( prdda>int(scale*trddb)) then
                print *,prdda,trddb
                stop '  prdb dimension error'
                end if
                call allocatela(nplrp(prdda),no_spartb,no_spartb)
                allocate(prdav(prdda)%v(db0(bjl/2)*db0(bjr/2)))
                prdav(prdda)%v=real(0,rkw)
                dimp(prdda)=dimb(btsc)
                prdav(prdda)%v(1:dimb(btsc))=me*trdbv(posb(btsc)+1:posb(btsc)+dimb(btsc))
                call zerola(nplrp(prdda),no_spartb)
                call laor(nplrp(prdda),nplrb(btsc),no_spartb,no_spartb)
            else if (.not.new.and.equ/=0) then
                call zerola(nplrbo(1),no_spartb)
                call laor(nplrbo(1),nplrp(equ),no_spartb,no_spartb)
                call laor(nplrbo(1),nplrb(btsc),no_spartb,no_spartb)
                call coefor(nplrbo(1),prdav(equ)%v,db0(bjl/2)*db0(bjr/2),&
                    nplrp(equ),me*trdbv(posb(btsc)+1:posb(btsc)+dimb(btsc)),nplrb(btsc),&
                        no_spartb,dimp(equ),dimb(btsc),nJTb,bjl,bjr)
                call laor(nplrp(equ),nplrb(btsc),no_spartb,no_spartb)
            end if 
        end if    
600  end do
     end do 
...
900  rewind(unit=21)
    ...
    do psc=1,prdda
        ...
        write(21) prdav(psc)%v(1:dimp(psc))
        if (allocated(prdav(psc)%v)) deallocate(prdav(psc)%v)
        write(21) (nplrp(psc)%lvect(bnpl)%lword(1:nwordsb),bnpl=1,no_spartb)
        call deallocatela(nplrp(psc),no_spartb)
    end do 
    write(21) posp(1:prdda)
    rewind(unit=21)
    deallocate(prdav)
    ...
    deallocate(posb)
    do psc=1,trddb
        call deallocatela(nplrb(psc),no_spartb)
    end do    
    deallocate(dimb)
    deallocate(nplrb)
    deallocate(trdbv)
    if (prdda/=0) then
    allocate(trdb(prdda))
    allocate(trdbv(prddat))
    allocate(manp(prdda))
    allocate(dimb(prdda))
    allocate(posb(prdda))
    allocate(nplrb(prdda))
    done=.true.
    idx=0
    nsc=0
930 idx=get_next_string(done)
    nsc=nsc+1
    trdb(nsc)(1:8)=string_order(idx)%string(1:8)
    manp(nsc)=idx
    if (done) go to 940
    go to 930
940 if (prdda/=nsc) then
        print *, ' prdda,nsc ',prdda,nsc
        stop '  prdda /= nsc  '
    end if
    end if
    read(21) trddb,dimb(1:trddb)
    nsc=0
    do psc=1,trddb
        posb(psc)=nsc
        read(21) trdbv(nsc+1:nsc+dimb(psc))
        nsc=nsc+dimb(psc)
        call allocatela(nplrb(psc),no_spartb,no_spartb)
        read(21) (nplrb(psc)%lvect(bnpl)%lword(1:nwordsb),bnpl=1,no_spartb)
    end do
    deallocate(dimb)
    do totlm=1,totlms
    do par=1,2
    write (99,*) totlm,par
    write (99,*) lmel(totlm,:,par)
    end do
    end do
    totlm=totlms
    end subroutine prd    
