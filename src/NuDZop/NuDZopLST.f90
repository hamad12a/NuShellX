...
real(kind=ri),dimension(:),allocatable:: order_V,order_W,order_VmW
integer:: n_int
...
contains
    ...
    pure function deltah(i,j)
    integer:: deltah
    integer,intent(in):: i,j
    integer:: pi,pj,pii,pij
    deltah=kdelta(sj2(i),sj2(j))
    if (deltah==0) return
    pi=2*sn(i)+sl(i)
    pj=2*sn(j)+sl(j)
    pii=1
    ...
    if (btest(pi,0)) pii=-1
    if (btest(pj,0)) pij=-1
    deltah=deltah*kdelta(pii,pij)  !*kdelta(pi,pj)
    end function deltah
    
    pure function deltae(i,j,k,l)
    integer:: deltae
    integer,intent(in):: i,j,k,l
    deltae=(deltah(i,j)-kdelta(i,j))*(deltah(k,j)-kdelta(k,j))
    end function deltae
    ...
    integer,intent(in):: J
    rho=1
    if (btest(J/2,0)) rho=-1
    end function rho
    
    pure function oprho(s,J)
    integer::oprho
...
subroutine SU3_v(vec,dvec,vec0,n,nd,n0)
implicit none
real(kind=ri),dimension(:),intent(inout):: vec,dvec,vec0
real(kind=ri),intent(inout):: n,nd,n0
integer:: ih,jh,il,ir,L
real(kind=ri):: d,m,mt,md,Ni,Nj,ml,mm,mmd,m0,md0,mt0
call init()
vec=0.0d0
dvec=0.0d0
do ih=1,no_shells
...
mt=mt+md*m*sqrt(real(6*(2*L+1),ri))*W6(2*sl(ih),2,2*sl(jh),2,2*L,4)/2.0
if (sl(jh)==L+1) mm=-sqrt(real((L+1)*(Nj+L+3),ri))
if (sl(ih)==L+1) mmd=-sqrt(real((L+1)*(Ni+L+3),ri))
if (sl(ih)==L-1) mmd=-sqrt(real(L*(Ni-L+2),ri))
if (sl(jh)==L-1) mm=-sqrt(real(L*(Ni-L+2),ri))
mm=-mm
if (btest((sl(jh)-sl(ih))/2,0)) mm=-mm
ml=ml+mmd*mm*sqrt(real(6*(2*L+1),ri))*W6(2*sl(ih),2,2*sl(jh),2,2*L,4)/2.0
end do
end if
il=(ih-1)*no_shells+jh
if (mt/=0.0d0) vec(il)=vec(il)+mt*&
 sqrt(real((sj2(ih)+1)*(sj2(jh)+1)*5,ri))*c9j(2*sl(ih),1,sj2(ih),2*sl(jh),1,sj2(jh),4,0,4)
...
m0=vec(1)/dvec(1)
vec0=dvec*m0-vec
n0=dot_product(vec0,vec0)
if (n0/=0.0) vec0=vec0/sqrt(n0)
n0=-nd/m0/m0
do ih=1,no_shells
do jh=1,no_shells     
il=(ih-1)*no_shells+jh
ir=(jh-1)*no_shells+ih
if (ih>jh) vec0(il)=-vec0(ir)
if (ih>jh.and.btest((sj2(ih)-sj2(jh))/2,0)) vec0(il)=-vec0(il)
!if (ih<jh.and.btest((sj2(ih)-sj2(jh))/2,0)) vec0(il)=-vec0(il)
end do
end do
...
contains
    ...
    implicit none
        ...
        real(kind=ri):: V,sumV,sumVt,sumVtp,sumVtm,delta12,delta34,vint,vintn,vintp,phase
        real(kind=8)::me,average,four2,limit,rcute,rcutm,rqqa,rqqb,rqqc,phs,delta13,delta24
        real(kind=8)::gamma2
        integer:: no_Op=0,one=1,lmid,rmid,tqlim,lamx,nz_op,lmids,rmids,rmida,lmida,loc
        integer:: sumJ,sumJp,sumJm,L,S,sig,sigx,l1,l2,l3,l4
        ...
        real(kind=8),dimension(:),allocatable:: veca,vecb,vecc
        integer,dimension(:),allocatable:: pl,pr,plo,pro
        integer,dimension(:,:,:,:),allocatable:: indx
        logical:: warn=.false.,done,warnp=.false.,swop
        ...
        allocate(veca(no_shells*no_shells))
        allocate(vecb(no_shells*no_shells))
        allocate(vecc(no_shells*no_shells))
        if (rem_Hcm)  call Hcm_v(veca,vecb)
        if (add_SU3)  call SU3_v(veca,vecb,vecc,rqqa,rqqb,rqqc)
        matrix=real(0,8)
        somega=real(0,8)
        alpha=real(0,8)
        omega=real(0,8)
...
!                                do sh4=1,no_shells
                           ...
                           ! check for j1=j2 or j3=j4 and limit to J+T EVEN
                                            if (ishell(sh1) == ishell(sh2)&
                                                ...
                                                if (btest(J/2,0)) then
                                                    if (.not.intgj.and.T==2) go to 5
                                                    if (intgj.and.T==0) go to 5
                                                else
                                                    if (.not.intgj.and.T==0) go to 5
                                                    if (intgj.and.T==2) go to 5
                                                end if
                                            end if
...
!                                            do T=Tmin,Tmax,2
                            ...
                            ! find V
                                                if (SDI) then
                                                  ...
                                                  V=findV(parti,j1,j2,j3,j4,J,T,loc,phs)
                                                end if
                                                if (V /= real(0,ri)) then
                                                    sumVt=sumVt+(J+1)*V
                                                    sumVtp=sumVtp+(J+1)*oprho(1,J)*V
                                                    sumVtm=sumVtm+(J+1)*oprho(-1,J)*V
                                                    sumJ=sumJ+(J+1)
                                                    sumJp=sumJp+(J+1)*oprho(1,J)
                                                    sumJm=sumJm+(J+1)*oprho(-1,J)
                                                end if
5                                            end do
                                            sumVt=sumVt/sumJ    
                                            if (sumJp/=0) then
                           ...
                           ! check for j1=j2 or j3=j4 and limit to J+T EVEN
                                            if (ishell(sh1) == ishell(sh2)&
                                                .or.ishell(sh3) == ishell(sh4)) then
                                                if (btest(J/2,0)) then
                                                    if (.not.intgj.and.T==2) go to 6
                                                    if (intgj.and.T==0) go to 6
                                                else
                                                    if (.not.intgj.and.T==0) go to 6
                                                    if (intgj.and.T==2) go to 6
                                                end if
                                            end if
                                             parti%shell(5)=J/2
                                             V=findV(parti,j1,j2,j3,j4,J,T,loc,phs,swop)
                                            if  (order_VmW(loc)==0.0d0.and..not.swop) then
                                                if (rho(J)==1) order_VmW(loc)=&
                                                order_VmW(loc)+phs&
                                                *deltah(sh1,sh3)*deltah(sh2,sh4)*(&
                                                (1-deltae(sh1,sh2,sh3,sh4))*sumVt+&
                                                deltae(sh1,sh2,sh3,sh4)*sumVtp)
                                                if (rho(J)==-1) order_VmW(loc)=&
                                                order_VmW(loc)+phs&
                                                *deltah(sh1,sh3)*deltah(sh2,sh4)*(&
                                                (1-deltae(sh1,sh2,sh3,sh4))*sumVt+&
                                                deltae(sh1,sh2,sh3,sh4)*sumVtm)
                                                order_W(loc)=order_W(loc)-order_VmW(loc)
                                             end if   
6                                            end  do                                             
                                          end do
                             !write out me
...
3             end do
4     end do
9        do sh1=min_ashells,max_ashells
!        do sh1=1,no_shells
                parti=0
                if (npintn) then
                    parti%shell(1)=sh1
...
!                                do sh4=1,no_shells
                                   ...
                                   if (npintn) then
                                        ...
                                        if (j3 == j4 .and. ishell(sh3) == &
                                          ishell(sh4)) delta34=sqrt(real(2,ri))
                                          lmn=max(abs(l1-l3),abs(l2-l4))
                                          lmx=min(abs(l1+l3),abs(l2+l4))
                                          max_lmo=max(max_lmo,lmx)
                                         do  lam=lmx,lmn,-2 
                                         do  tau=0,taux,2
                                         do  sig=0,sigx,2
                           ...
                           ! check for j1=j2 or j3=j4 and limit to J+T EVEN
                            ...
                            ! find V
                                                if (SDI) then
                                                  ...
                                                  V=findV(parti,j1,j2,j3,j4,J,T,loc,phs)
                                                  if (V==0.0.and.p1*p2*p3*p4==1)then
                                                   warn=.true.
                                                   write(21,*) sh1,sh2,sh3,sh4,J/2,T/2
                                                  end if 
                                                  if (V/=0.0.and.p1*p2*p3*p4==-1) warnp=.true.
                                                end if
                                                V=order_W(loc)*phs
                                                if (V /= real(0,ri)) then
                                                    gamma2=sqrt(real((j1+1)*(j2+1)*(j3+1)*(j4+1),rc))*&
                                                    real((L+1)*(S+1),rc)*&
                                                    ninej(l1,1,j1,l2,1,j2,L,S,J)/delta12/delta34*&
                                                    ninej(l3,1,j3,l4,1,j4,L,S,J)*sqrt(2.0d0)
                                                    phase=real(1,rc)
                                                    if (btest((l1+l4-L-lam)/2,0)) phase=-phase
                                                    vint=V*gamma2*real((L+1),rc)*sqrt(real((lam+1),rc))&
                                                         *phase*racah(l1,l2,l3,l4,L,lam)&
                                                         *delta12*delta34/four2
                                                    if (.not.npintn) then
                                                        if (btest((2-T-tau)/2,0)) vint=-vint
                                                        vint=vint*real((T+1),rc)*sqrt(real((tau+1),rc))&
                                                        *racah(1,1,1,1,T,tau)
                                                    end if
                                                    if (btest((2-S-sig)/2,0)) vint=-vint
                                                    vint=vint*real(T+1,rc)*sqrt(real(sig+1,rc))*&
                                                        racah(1,1,1,1,S,sig)
                                                    sumV=sumV+vint
                                                end if
                                                end if
                             ...
                             !write out me
                                          ...
                                          rmids=(sh4-1)*no_shells+sh2
                                          matrix(lmid,rmid,lam/2,tau/2,sig/2)=sumV
                                          somega(lmid,rmid,lam/2,tau/2,sig/2)=sumV
                                         end do
                                         end do
                                         end do
                                         end if
...
40     end do
    ...
    do tau=0,1
        ...
        call tred2(matrix(:,:,lam,tau,sig),no_shells*no_shells,no_shells*no_shells,alpha(:,lam,tau,sig),beta)
        tqlim=2000
        call tqli(alpha(:,lam,tau,sig),beta,no_shells*no_shells,no_shells*no_shells,matrix(:,:,lam,tau,sig),tqlim)
        if (lam==1.and.tau==0.and.sig==0) then
            if (rem_Hcm) &
            call remove_Hcm(matrix(:,:,lam,tau,sig),alpha(:,lam,tau,sig),indx(:,lam,tau,sig),no_shells,veca,vecb,limit)
        end if                             

        if (lam==2.and.tau==0.and.sig==0) then
            if (add_SU3)then
                    sh1=minval(minloc(abs(alpha(:,lam,tau,sig))))
                    if (abs(alpha(sh1,lam,tau,sig)/sqrt(5.0d0))<.01) then
                         alpha(sh1,lam,tau,sig)=-sqrt(5.0d0)*rqqa
                         matrix(:,sh1,lam,tau,sig)=veca(:)
                    else
                        print *, ' No place to add Q+.'
                    end if
             end if
            if (add_SU3)then
                    sh1=minval(minloc(abs(alpha(:,lam,tau,sig))))
                    if (abs(alpha(sh1,lam,tau,sig)/sqrt(5.0d0))<.01) then
                         alpha(sh1,lam,tau,sig)=-sqrt(5.0d0)*rqqb
                         matrix(:,sh1,lam,tau,sig)=vecb(:)
                    else
                        print *, ' No place to add Q-.'
                    end if
             end if
            if (add_SU3)then
                    sh1=minval(minloc(abs(alpha(:,lam,tau,sig))))
                    if (abs(alpha(sh1,lam,tau,sig)/sqrt(5.0d0))<.01) then
                         alpha(sh1,lam,tau,sig)=-sqrt(5.0d0)*rqqc
                         matrix(:,sh1,lam,tau,sig)=vecc(:)
                    else
                        print *, ' No place to add Q0.'
                    end if
             end if
        end if                             

        call indexx(no_shells*no_shells,-abs(alpha(:,lam,tau,sig)),indx(:,lam,tau,sig))      
        
        write (50,*) ' Lambda,tau,sigma',lam,tau,sig
        write (55,*) ' Lambda,tau,sigma',lam,tau,sig
        write (50,'(10f8.4)') alpha(indx(1:no_shells*no_shells,lam,tau,sig),lam,tau,sig)  &
            /sqrt(real(2*lam+1,8))/sqrt(real(2*tau+1,8))/sqrt(real(2*sig+1,rc))
        do i=1,no_shells*no_shells
        if (abs(alpha(indx(i,lam,tau,sig),lam,tau,sig))/sqrt(real(2*lam+1,8))&
                /sqrt(real(2*tau+1,8))/sqrt(real(2*sig+1,rc))>0.0000001) then
            lalpha(indx(i,lam,tau,sig),lam,tau,sig)=.true.
            write(55,'(a5,f14.9)') ' e =',alpha(indx(i,lam,tau,sig),lam,tau,sig)&
                    /sqrt(real(2*lam+1,8))/sqrt(real(2*tau+1,8))/sqrt(real(2*sig+1,rc))
            if (lalpha(indx(i,lam,tau,sig),lam,tau,sig)) then
            do sh1=1,no_shells
            do sh2=1,no_shells
            lmid=(sh1-1)*no_shells+sh2
            if (abs(matrix(lmid,indx(i,lam,tau,sig),lam,tau,sig))>0.00000001) then
                lmatrix(lmid,indx(i,lam,tau,sig),lam,tau,sig)=.true.
               write(55,'(2i4,f14.9)') sh1,sh2,matrix(lmid,indx(i,lam,tau,sig),lam,tau,sig)
            end if
            end do  
            end do
            end if
    ...
    do tau=0,1
        read (55,*) 
        write(56,*) ' Lambda,tau',lam,tau
        do i=1,no_shells*no_shells
        if (lalpha(indx(i,lam,tau,sig),lam,tau,sig)) then
            read(55,'(6x,f14.9)') alpha(indx(i,lam,tau,sig),lam,tau,sig)
            if (lam>0) then
            if (do_cut.and.abs(alpha(indx(i,lam,tau,sig),lam,tau,sig))<rcute)  &
                alpha(indx(i,lam,tau,sig),lam,tau,sig)=0.0d0
            end if  
            write(56,'(a5,f14.9)') 'e = ',alpha(indx(i,lam,tau,sig),lam,tau,sig)
            alpha(indx(i,lam,tau,sig),lam,tau,sig)=alpha(indx(i,lam,tau,sig),lam,tau,sig) &
                 *sqrt(real(2*lam+1,8))*sqrt(real(2*tau+1,8))
            if (lalpha(indx(i,lam,tau,sig),lam,tau,sig)) then 
            do sh1=1,no_shells
            do sh2=1,no_shells
            lmid=(sh1-1)*no_shells+sh2
            if (lmatrix(lmid,indx(i,lam,tau,sig),lam,tau,sig)) then
                read(55,'(2i4,f14.9)') sh3,sh4,matrix(lmid,indx(i,lam,tau,sig),lam,tau,sig)
                if (lam>0)  then
                if (do_cut.and.abs(matrix(lmid,indx(i,lam,tau,sig),lam,tau,sig))<rcutm) &
                    matrix(lmid,indx(i,lam,tau,sig),lam,tau,sig)=0.d0
                end if
                if (abs(matrix(lmid,indx(i,lam,tau,sig),lam,tau,sig))>=rcutm.and.lam>0) then
                    sh1sh2(lmid)=.true. 
                    write(56,'(2i4,f14.9)') sh1,sh2,matrix(lmid,indx(i,lam,tau,sig),lam,tau,sig)
                end if   
                if (lam==0) then
                    sh1sh2(lmid)=.true. 
                    write(56,'(2i4,f14.9)') sh1,sh2,matrix(lmid,indx(i,lam,tau,sig),lam,tau,sig)
         ...
         omega(:,:,lam,tau,sig)=0.0d0
         do sh1=1,no_shells*no_shells
         omega(sh1,sh1,lam,tau,sig)=alpha(sh1,lam,tau,sig)
         end do
         omega(:,:,lam,tau,sig)=matmul(matrix(:,:,lam,tau,sig),&
            matmul(omega(:,:,lam,tau,sig),transpose(matrix(:,:,lam,tau,sig))))
    end do
     do sh1=1,no_shells
     do sh2=1,no_shells
     lmid=(sh1-1)+sh2
    ...
    no_Op=no_Op+1
     ...
     do tau=0,taux,2
        V=omega(lmid,rmid,lam/2,tau/2,sig/2)
        if (V /= real(0,ri)) then
           phase=real(1,rc)
     ...
     order_V(no_Op)=sumV+order_VmW(no_Op)
     if (npoutn) then
        if (T==2) then
        max_op=max_op+1
...
600    print *, 'Error writing ', Nucleus//xtra//'tint.txt'
    ...
    function findV(parti,j1,j2,j3,j4,J,T,pos,phs,swop)
    implicit none
        type(spartition),intent(in):: parti
        real(kind=ri):: findV
        integer,intent(in):: j1,j2,j3,j4,J,T
        integer,intent(out),optional::pos
        real(kind=ri),intent(out),optional:: phs
        logical,intent(out),optional:: swop 
...
10         findp=bfind(order_int,n_int,part)
           if (findp /= 0) then
               findV=order_V(findp)
               if (present(pos)) pos=findp
               if (present(phs)) phs=1.0d0
               if (present(swop).and..not.done) swop=.false.
               return
           else
               perm_part=part
               ...
               if (findp /= 0) then
                   ...
                   lhs=.true.
                   if (present(pos)) pos=findp
                   if (present(phs)) phs=phase
                   if (present(swop)) swop=.true.
                   return
               else
                   perm_part=part
...
!****************************************************************************
    ...
    allocate(order_W(noint))
    allocate(order_VmW(noint))
    order_V=0.0d0
    order_W=0.0d0
    order_VmW=0.0d0
