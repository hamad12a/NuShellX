...
contains
    ...
    implicit none
        ...
        read(cute,*) rcute
        read(cutm,*) rcutm
        print *, ' e cut sqrt((2lambda+1)(2tau+1))*',rcute 
        print *, ' v cut',rcutm        
        else
            rcute=.0000000001
            rcutm=.0000000001
        ...
        allocate(plo(notree))
        allocate(pro(notree))
        allocate(matrix(no_shells*no_shells,no_shells*no_shells,0:max_sj2u,0:1))
        allocate(lmatrix(no_shells*no_shells,no_shells*no_shells,0:max_sj2u,0:1))
        allocate(sh1sh2(no_shells*no_shells))
        allocate(lalpha(no_shells*no_shells,0:max_sj2u,0:1))
        allocate(alpha(no_shells*no_shells,0:max_sj2u,0:1))
        allocate(indx(no_shells*no_shells,0:max_sj2u,0:1))
        allocate(somega(no_shells*no_shells,no_shells*no_shells,0:max_sj2u,0:1))
        allocate(omega(no_shells*no_shells,no_shells*no_shells,0:max_sj2u,0:1))
                           ...
                           ! check for j1=j2 or j3=j4 and limit to J+T EVEN
                            ...
                            ! find V
                             ...
                             !write out me
                                          ...
                                          matrix(lmid,rmid,lam/2,tau/2)=sumV
                                          somega(lmid,rmid,lam/2,tau/2)=sumV
                                         end do
                                         end do
                                         end if
10                             end do
...
40     end do
    ...
    do tau=0,1
        ...
        call tred2(matrix(:,:,lam,tau),no_shells*no_shells,no_shells*no_shells,alpha(:,lam,tau),beta)
        tqlim=2000
        call tqli(alpha(:,lam,tau),beta,no_shells*no_shells,no_shells*no_shells,matrix(:,:,lam,tau),tqlim)
        if (lam==1.and.tau==0) then
            if (rem_Hcm) &
            call remove_Hcm(matrix(:,:,lam,tau),alpha(:,lam,tau),indx(:,lam,tau),no_shells,veca,vecb,limit)
        end if                             

        if (lam==2.and.tau==0) then
            if (add_SU3)then
                    sh1=minval(minloc(abs(alpha(:,lam,tau))))
                    if (abs(alpha(sh1,lam,tau)/sqrt(5.0d0))<.01) then
                         alpha(sh1,lam,tau)=-sqrt(5.0d0)*rqqa
                         matrix(:,sh1,lam,tau)=veca(:)
                    else
            ...
            if (add_SU3)then
                    sh1=minval(minloc(abs(alpha(:,lam,tau))))
                    if (abs(alpha(sh1,lam,tau)/sqrt(5.0d0))<.01) then
                         alpha(sh1,lam,tau)=-sqrt(5.0d0)*rqqb
                         matrix(:,sh1,lam,tau)=vecb(:)
                    else
                        print *, ' No place to add Q-.'
                    end if
             end if
            if (add_SU3)then
                    sh1=minval(minloc(abs(alpha(:,lam,tau))))
                    if (abs(alpha(sh1,lam,tau)/sqrt(5.0d0))<.01) then
                         alpha(sh1,lam,tau)=-sqrt(5.0d0)*rqqc
                         matrix(:,sh1,lam,tau)=vecc(:)
                    else
        ...
        call indexx(no_shells*no_shells,-abs(alpha(:,lam,tau)),indx(:,lam,tau))      
        
        write (50,*) ' Lambda,tau',lam,tau
        write (55,*) ' Lambda,tau',lam,tau
        write (50,'(10f8.3)') alpha(indx(1:no_shells*no_shells,lam,tau),lam,tau)  &
            /sqrt(real(2*lam+1,8))/sqrt(real(2*tau+1,8))
        do i=1,no_shells*no_shells
        if (abs(alpha(indx(i,lam,tau),lam,tau))/sqrt(real(2*lam+1,8))&
                /sqrt(real(2*tau+1,8))>0.0000001) then
            lalpha(indx(i,lam,tau),lam,tau)=.true.
            write(55,'(a5,f14.9)') ' e =',alpha(indx(i,lam,tau),lam,tau)&
                    /sqrt(real(2*lam+1,8))/sqrt(real(2*tau+1,8))
            if (lalpha(indx(i,lam,tau),lam,tau)) then
            do sh1=1,no_shells
            do sh2=1,no_shells
            lmid=(sh1-1)*no_shells+sh2
            if (abs(matrix(lmid,indx(i,lam,tau),lam,tau))>0.00000001) then
                lmatrix(lmid,indx(i,lam,tau),lam,tau)=.true.
                write(55,'(2i4,f14.9)') sh1,sh2,matrix(lmid,indx(i,lam,tau),lam,tau)
            end if
            end do  
            end do
            end if
    ...
    do tau=0,1
        ...
        do i=1,no_shells*no_shells
        if (lalpha(indx(i,lam,tau),lam,tau)) then
            read(55,'(6x,f14.9)') alpha(indx(i,lam,tau),lam,tau)
            if (lam>0) then
            if (do_cut.and.abs(alpha(indx(i,lam,tau),lam,tau))<rcute)  &
                alpha(indx(i,lam,tau),lam,tau)=0.0d0
            end if  
            write(56,'(a5,f14.9)') 'e = ',alpha(indx(i,lam,tau),lam,tau)
            alpha(indx(i,lam,tau),lam,tau)=alpha(indx(i,lam,tau),lam,tau) &
                 *sqrt(real(2*lam+1,8))*sqrt(real(2*tau+1,8))
            if (lalpha(indx(i,lam,tau),lam,tau)) then 
            do sh1=1,no_shells
            do sh2=1,no_shells
            lmid=(sh1-1)*no_shells+sh2
            if (lmatrix(lmid,indx(i,lam,tau),lam,tau)) then
                read(55,'(2i4,f14.9)') sh3,sh4,matrix(lmid,indx(i,lam,tau),lam,tau)
                if (lam>0)  then
                if (do_cut.and.abs(matrix(lmid,indx(i,lam,tau),lam,tau))<rcutm) &
                    matrix(lmid,indx(i,lam,tau),lam,tau)=0.d0
                end if
                if (abs(matrix(lmid,indx(i,lam,tau),lam,tau))>=rcutm.and.lam>0) then
                    sh1sh2(lmid)=.true. 
                    write(56,'(2i4,f14.9)') sh1,sh2,matrix(lmid,indx(i,lam,tau),lam,tau)
                end if   
                if (lam==0) then
                    sh1sh2(lmid)=.true. 
                    write(56,'(2i4,f14.9)') sh1,sh2,matrix(lmid,indx(i,lam,tau),lam,tau)
         ...
         omega(:,:,lam,tau)=0.0d0
         do sh1=1,no_shells*no_shells
         omega(sh1,sh1,lam,tau)=alpha(sh1,lam,tau)
         end do
         omega(:,:,lam,tau)=matmul(matrix(:,:,lam,tau),&
            matmul(omega(:,:,lam,tau),transpose(matrix(:,:,lam,tau))))
    end do
     do sh1=1,no_shells
     do sh2=1,no_shells
     lmid=(sh1-1)+sh2
