...
! end sun
        ...
        write (10,'(100i4)') (min_cSNo%shell(shn),shn=1,no_shells)
        write (10,'(100i4)') (max_cSNo%shell(shn),shn=1,no_shells)
        write (10,'(100i4)') (min_MS_Non%shell(shn),shn=1,no_shells)
        write (10,'(100i4)') (max_MS_Non%shell(shn),shn=1,no_shells)
        write (10,'(i4)') output_control
...
! end sun
        ...
        write (10,'(100i4)') (min_cSNo%shell(shn),shn=1,no_shells)
        write (10,'(100i4)') (max_cSNo%shell(shn),shn=1,no_shells)
        write (10,'(100i4)') (min_MS_Nop%shell(shn),shn=1,no_shells)
        write (10,'(100i4)') (max_MS_Nop%shell(shn),shn=1,no_shells)
        write (10,'(i4)') output_control
...
contains
    ...
    subroutine input_model_shell(inExt)
        ...
        read (10,*,err=30,end=31) (min_MS_Nop%shell(shn),shn=1,nop),(min_MS_Non%shell(shn),shn=1,non)
        call read_comments_model()
        read (10,*,err=30,end=31) (max_MS_Nop%shell(shn),shn=1,nop),(max_MS_Non%shell(shn),shn=1,non)
...
!        end if
        ...
        if (nucleon(1)=='i') then
            ...
            nmpp=-abs(nP)
            nmpn=abs(nN)
            nja=sum(sj2(no_shellsp()+1:no_shells))/(no_shells-no_shellsp())
            pja=sum(sj2(1:no_shellsp()))/no_shellsp()
            if (nN*nja >= nP*pja.or.&
...
100     write(6,'(a48)',advance='no') '  Do you want to apply sp energy truncation y/n '
        read(5,'(a1)') ans
        if (ans=='y'.or.ans=='Y') then
        ...
        if (help) then
            write(6,*) '  This is an experimental option based on ideas in PRC,54,1641(1996)'
            write(6,*) '  It is applied only to pn interaction and p and n orbitalss must be the same.'
        ...
        read(5,'(a1)') ans
        if (ans=='y'.or.ans=='Y') then
            write(6,'(a41)',advance='no') '  Absolute energy cut e (MeV) , ev cut % '
            read(5,*) cutBZ,cutBZm
            BZ='Z'
            cutBZm=cutBZm/100.0
        end if
...
120     write(6,*) ' Minumum , Maximum , Delta 2*J n ',min_cJ2n,max_cJ2n,del_cJ2n
        write(6,'(a13)',advance='no') '  Accept y/n '
        read(5,'(a1)') ans
...
!            end if
        ...
        res=run('NuDiop',Nucleus//xtra//' '//chcutBZ//' '//chcutBZm)
        if (res==-1) stop
        write(56,*)'NuDiop'//' ',Nucleus//xtra//' '//chcutBZ//' '//chcutBZm
        !write(6,*)
...
contains
    ...
    subroutine Transition()
        ...
        character(len=5):: Nucleusi,Nucleusf
